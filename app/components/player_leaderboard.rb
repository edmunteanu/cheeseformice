class PlayerLeaderboard < ViewComponent::Base
  include PlayerRankChange

  CATEGORY_ATTRIBUTES = {
    normal: %i[saved_mice cheese_gathered firsts bootcamp rounds_played],
    racing: %i[racing_finished_maps racing_podiums racing_firsts racing_rounds_played],
    survivor: %i[survivor_shaman_rounds survivor_mice_killed survivor_survived_rounds survivor_rounds_played],
    defilante: %i[defilante_finished_maps defilante_points defilante_rounds_played]
  }
  PODIUM_COUNT = 3

  attr_reader :players, :category, :statistic

  delegate :humanized_title, :player_path, to: :helpers

  def initialize(players:, category:, statistic:, time_range:, current_page:)
    @players = players
    @category = category
    @statistic = statistic
    @time_range = time_range
    @current_page = current_page
  end

  def first_page?
    @current_page == 1
  end

  def score_statistic?
    %i[normal_score racing_score survivor_score defilante_score].include?(statistic)
  end

  def category_rounds_played_attribute
    base = "rounds_played"
    base.prepend("#{category}_") unless category == :normal
    base.to_sym
  end

  def current_rank(player, index)
    if score_statistic? && show_player_data?
      number_with_delimiter(player.public_send(:"#{category}_rank"))
    else
      podium_offset = first_page? ? PODIUM_COUNT : 0
      statistic_rank = podium_offset + (index + 1) + (@current_page - 1) * Pagy::DEFAULT[:limit]
      number_with_delimiter(statistic_rank)
    end
  end

  def show_player_data?
    @time_range == Player::TIME_RANGE_DEFAULT
  end

  def category_score(player)
    score = show_player_data? ? player.public_send(score_attribute) : change_log(player).public_send(score_attribute)
    number_with_delimiter(score)
  end

  def category_attribute_value(player, attribute)
    value = show_player_data? ? player.public_send(attribute) : change_log(player).public_send(attribute)
    number_with_delimiter(value)
  end

  def statistic_value(player)
    value = show_player_data? ? player.public_send(statistic) : change_log(player).public_send(statistic)
    number_with_delimiter(value)
  end

  def category_rounds_played_value(player)
    value = if show_player_data?
              player.public_send(category_rounds_played_attribute)
    else
              change_log(player).public_send(category_rounds_played_attribute)
    end

    number_with_delimiter(value)
  end

  private

  def score_attribute
    :"#{category}_score"
  end

  def change_log(player)
    case @time_range
    when :past_day
      player.change_logs_past_day
    when :past_7_days
      player.change_logs_past_7_days
    when :past_30_days
      player.change_logs_past_30_days
    end
  end
end
