...
RSpec.describe PlayersController do
    ...
    describe "performance" do
      context "when loading the default (score-statistic) leaderboard page" do
        it "does not perform too many queries" do
          expect { get leaderboard_path }.to make_database_queries(count: 1, matching: /SELECT/)
        end
      end
      ...
      context "when selecting a non-score statistic" do
        it "does not perform too many queries" do
          expect { get leaderboard_path, params: { statistic: "saved_mice" } }
            .to make_database_queries(count: 1, matching: /SELECT/)
        end
      end
  ...
  describe "#show" do
    ...
    describe "performance" do
      context "when the player has a previous day change log" do
        ...
        it "does not perform too many queries" do
          expect { get player_path(player) }.to make_database_queries(count: 5, matching: /SELECT/)
        end
      end
  ...
  describe "#search" do
    ...
    before { allow(SearchService).to receive(:new).and_call_original }
